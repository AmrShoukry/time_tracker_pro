...
const Focus = () => {
    ...
    useEffect(() => {
        ...
        if (isRunning && time > 0)
        {
            ...
            .then(data => {
                data["completed"] = 1
                updateRequest(`http://localhost:8000/projects/${currentTaskId}`, data)
                reloadPage()
            });
            ...
            <div className="container">
                <div id="selection">
                    ...
                    <form action="" id="taskForm">
                        <div>
                            <p>Choose a task</p>
                            <p onClick={(e) => handleSelected(e)} className='selected' id={currentTaskId}>
                                {data && data.map((project) => {
                                    if (project["completed"] === 0 && firstItem === "There's no remaining projects")
                                    {
                            ...
                            <ul>
                                {data && data.map((project) => {
                                    ...
                                    {
                                        return (
                                            <li onClick={(e) => handleSelection(e, project["id"])}>{project["headline"]}</li>
                                        );
                                    }
                                    })
                                }
