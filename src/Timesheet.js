...
const MyTimesheet = () => {
    ...
    useEffect(() => {
        ...
        .then(data => {
            ...
            console.log(`Current Month is ${currentMonth} Current Year is ${currentYear}`)
            data.forEach(project => {
                ...
                if (isValid(from, to, currentTime))
                {
                    ...
                    while(iterativeYear < currentYear && iterativeMonth > closestMonth)
                    {
                        modified = 1
                        let iterativeDate = new Date(iterativeYear, iterativeMonth, 1)
                        const firstDayOfNextMonth = moment([iterativeYear, iterativeMonth]).add(1, 'months');
                        const lastMomentOfMonth = firstDayOfNextMonth.subtract(1, 'milliseconds');
                        const secondsDifference = lastMomentOfMonth.diff(iterativeDate, 'seconds');
                        durations[iterativeMonth - currentMonth - 1] += parseInt(secondsDifference / 60 / 60)
                        setMonths(durations)
                    ...
                    while(iterativeYear === currentYear)
                    {
                        while(iterativeMonth < closestMonth)
                        {
                            ...
                            const firstDayOfNextMonth = moment([iterativeYear, iterativeMonth]).add(1, 'months');
                            const lastMomentOfMonth = firstDayOfNextMonth.subtract(1, 'milliseconds');
                            const secondsDifference = lastMomentOfMonth.diff(compared, 'seconds');
                            durations[11 - (currentMonth - iterativeMonth)] += parseInt(secondsDifference / 60 / 60)
                            setMonths(durations)
                        ...
                        if (iterativeMonth === closestMonth)
                        {
                            if (modified === 0)
                            {
                                const secondsDifference = parseInt((closestDate - startingPoint)/1000)
                                durations[11 - (currentMonth - iterativeMonth)] += parseInt(secondsDifference / 60 / 60)
                                setMonths(durations)
                            }
                            else
                            {
                                let iterativeDate = new Date(iterativeYear, iterativeMonth, 1)
                                const secondsDifference = parseInt((closestDate - iterativeDate)/1000)
                                durations[11 - (currentMonth - iterativeMonth)] += parseInt(secondsDifference / 60 / 60)
                                setMonths(durations)
                            }
                            break;
